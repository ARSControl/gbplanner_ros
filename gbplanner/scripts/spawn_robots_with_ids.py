...
def spawn_robots():
    ...
    try:
        ...
        robots = [{'name': 'rmf_obelix', 'base_name': 'rmf_obelix', 'id': 0, 'x': 0.0, 'y': 0.0, 'z': 0.5}]
    ...
    for robot in robots:
        ...
        robot_id = str(robot.get('id', 0))
        robot_x = str(robot.get('x', 0.0))
        robot_y = str(robot.get('y', 0.0))
        robot_z = str(robot.get('z', 0.5))
        
        rospy.loginfo("Spawning robot: %s with id %s at (%s, %s, %s)", robot_name, robot_id, robot_x, robot_y, robot_z)
