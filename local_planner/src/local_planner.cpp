...
{
  // Takes the time when the odometry is received
  odomTime = odom->header.stamp.toSec();

  // Gets roll, pith and yaw from the orientation contained in the odometry msg
  double roll, pitch, yaw;
  geometry_msgs::Quaternion geoQuat = odom->pose.pose.orientation;
  tf::Matrix3x3(tf::Quaternion(geoQuat.x, geoQuat.y, geoQuat.z, geoQuat.w)).getRPY(roll, pitch, yaw);

  // Defines the current pose of the robot
  vehicleRoll = roll;
  vehiclePitch = pitch;
  vehicleYaw = yaw;
...
{
  // If i don't want to use the terrain analysis
  if (!useTerrainAnalysis) {
    // Clear the old laser cloud and fill it with the new one arrived
    laserCloud->clear();
    ...
    // Takes the number of points of the pointcloud received
    int laserCloudSize = laserCloud->points.size();
    // Iterate over all the poins of the pointcloud
    for (int i = 0; i < laserCloudSize; i++) {
      ...
      // Takes the coordinates of the actual point of the pointcloud
      ...
      // Compute the distance btw the point and the current robot position
      float dis = sqrt((pointX - vehicleX) * (pointX - vehicleX) + (pointY - vehicleY) * (pointY - vehicleY));
      // fill the laserCloudCrop with points of the pointcloud closer to the robot less than a certain threshold distance
...
void readStartPaths()
{
  ...
  if (filePtr == NULL) {
    printf ("\n[READSTARTPATHS] Cannot read input files, exit.\n\n");
    exit(1);
  }
...
void readPaths()
{
  ...
  FILE *filePtr = fopen(fileName.c_str(), "r");
  if (filePtr == NULL) {
    printf ("\n[READPATHS]Cannot read input files, exit.\n\n");
    exit(1);
  }
...
void readPathList()
{
  ...
  FILE *filePtr = fopen(fileName.c_str(), "r");
  if (filePtr == NULL) {
    printf ("\n[READPATHLISST]Cannot read input files, exit.\n\n");
    exit(1);
  }
...
void readCorrespondences()
{
  ...
  FILE *filePtr = fopen(fileName.c_str(), "r");
  if (filePtr == NULL) {
    printf ("\n[CORRESPONDENCES]Cannot read input files, exit.\n\n");
    exit(1);
  }
